class Visualiser {
    ...
    static drawLevel(ctx, level, left, top, width, height, outputLabels) {
        ...
        // Drawing the connections for the net
        for (let i = 0; i < inputs.length; i++) {
            for (let j = 0; j < outputs.length; j++) {
                ...
                // Colour coding the connections (yellow == close to 1, blue == close to -1 and alpha == close to 0)
                ctx.strokeStyle = getRGBA(weights[i][j]);
                ctx.stroke();
            }
        }

        // Drawing the input circles
        for (let i = 0; i < inputs.length; i++) {
